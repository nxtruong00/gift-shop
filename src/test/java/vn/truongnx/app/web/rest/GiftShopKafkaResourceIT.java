...
import vn.truongnx.app.config.EmbeddedKafka;
import vn.truongnx.app.config.KafkaSseConsumer;
import vn.truongnx.app.config.KafkaSseProducer;

@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
@EmbeddedKafka
class GiftShopKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gift-shop-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gift-shop-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/gift-shop-kafka/unregister"));
                return;
            }
        }
